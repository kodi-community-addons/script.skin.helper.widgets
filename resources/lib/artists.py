...
class Artists(object):
    ...
    def recommended(self):
        ...
        all_items = self.artutils.kodidb.artists(sort=kodi_constants.SORT_RATING,
                                                filters=[], limits=(0, self.options["limit"]))
        return process_method_on_list(self.process_artist, all_items)

    def recent(self):
        ''' get recently added artists '''
        all_items = self.artutils.kodidb.artists(sort=kodi_constants.SORT_DATEADDED, filters=[],
                                                limits=(0, self.options["limit"]))
        return process_method_on_list(self.process_artist, all_items)

    def random(self):
        ''' get random artists '''
        all_items = self.artutils.kodidb.artists(sort=kodi_constants.SORT_RANDOM, filters=[],
                                                limits=(0, self.options["limit"]))
        return process_method_on_list(self.process_artist, all_items)
    ...
    def process_artist(self, item):
        ...
        if self.enable_artwork:
            extend_dict(item, self.artutils.get_music_artwork(item["label"][0]))
