#!/usr/bin/python
# -*- coding: utf-8 -*-
from utils import ADDON_ID, process_method_on_list, create_main_entry
...
class Musicvideos(object):
    ...
    def listing(self):
        ...
        all_items = [ 
            (self.addon.getLocalizedString(32061), "inprogress&mediatype=musicvideos&tag=%s"%tag, "DefaultTvShows.png"), 
            (self.addon.getLocalizedString(32040), "recent&mediatype=musicvideos&tag=%s"%tag, "DefaultRecentlyAddedmusicvideos.png"),
            (self.addon.getLocalizedString(32062), "random&mediatype=musicvideos&tag=%s"%tag, "DefaultTvShows.png"),
            (xbmc.getLocalizedString(10134), "favourites&mediatype=musicvideos&tag=%s"%tag, "DefaultMovies.png")
            ]
    ...
    def inprogress(self):
        ...
        return self.artutils.kodidb.musicvideos(sort=kodi_constants.SORT_LASTPLAYED, filters=[kodi_constants.FILTER_INPROGRESS], 
            limits=(0,self.options["limit"]))
